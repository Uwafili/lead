      ...
      <nav class="space-x-6 flex items-center">
        ...
        @guest
          ...
          <a href="{{ route('register') }}"
            class="inline-flex items-center hover:underline font-medium transition-colors text-white duration-200">
            <!-- Register Icon -->
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1 text-yellow-700" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
          ...
          <a href="{{ route('login') }}"
            class="inline-flex items-center hover:underline font-medium transition-colors duration-200 text-white">
            <!-- Login Icon -->
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1 text-yellow-700" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M15 12H3m0 0l4-4m-4 4l4 4M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Login
          </a>
        @endguest
        ...
        @auth
          ...
          @else
            <p
              class="px-6 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200">
              <a href="{{route('updateTar')}}">Update Tariff</a></p>
    ...
    <div class="flex flex-1">
      ...
      <aside class="w-28 bg-white bg-opacity-80 shadow-lg flex flex-col items-center py-8 space-y-8">
        ...
        <a href="{{route('currTarView')}}" class="flex flex-col items-center group">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-blue-600 group-hover:text-blue-800" fill="none"
        ...
        {{-- <a href="{{route('Service')}}" class="flex flex-col items-center group">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-orange-500 group-hover:text-orange-700" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
        ...
        @auth
          @if(Auth::user() && Auth::user()->usertype == 'admin')
            ...
            <a href="{{route('Ad_ViewTariff')}}" class="flex flex-col items-center group">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-purple-500 group-hover:text-purple-700" fill="none"
                viewBox="0 0 24 24" stroke="currentColor">
            ...
            <a href="{{route('admin.users')}}" class="flex flex-col items-center group">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-yellow-600 group-hover:text-yellow-800" fill="none"
                viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @php
          $chatUnread = auth()->check() ? \App\Models\ChatMessage::where('receiver_id', auth()->id())->whereNull('read_at')->count() : 0;
         @endphp
        <a href="{{ route('chat.index') }}" class="relative flex flex-col items-center group">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-red-600 group-hover:text-red-800" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h8m-8 4h6" />
          </svg>
          @if($chatUnread > 0)
            <span
              class="absolute -top-2 -right-0 inline-flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-[11px] text-white">{{ $chatUnread }}</span>
          @endif
          <span class="text-xs mt-1 text-gray-700">Chat</span>
        </a>
      </aside>
  @endauth
  ...
  <footer class="bg-black bg-opacity-90 text-white">
    <div class="container mx-auto px-4 py-6 flex flex-col md:flex-row justify-between items-center">
      ...
      <div class="mt-4 md:mt-0 space-x-4">
        <a href="https://www.leadway.com/privacy-policy/" class="hover:text-orange-400 text-sm transition">Privacy
          Policy</a>
        <a href="https://www.leadway.com/wp-content/uploads/2019/07/TERMSdoc.pdf"
          class="hover:text-orange-400 text-sm transition">Terms of Service</a>
