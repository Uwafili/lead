self.onmessage = function (e) {
  ...
  /* 🔥 BUILD EXACT LOOKUP MAP (O(1)) */
  const EXACT_MAP = new Map();
  for (const item of dictionary) {
    const norm = normalize(item);
    if (norm && !EXACT_MAP.has(norm)) {
  ...
  function smartFuzzyMatch(input, target) {
    ...
    const levScore = weightedSimilarity(input, target);
    const gramScore = ngramSimilarity(input, target);
    const soundScore =
  ...
  /* -------- TOP-N BEST MATCHES (FAST PATH ENABLED) ------- */
  function topMatches(input, list, limit = 10, minScore = 0.6) {
    const normInput = normalize(input);

    /* ⚡ O(1) EXACT MATCH */
    if (EXACT_MAP.has(normInput)) {
      return [{ value: EXACT_MAP.get(normInput), score: 1 }];
    }
    ...
    return list
      .map(item => ({
        value: item,
        score: Number(smartFuzzyMatch(normInput, item).toFixed(3))
      }))
